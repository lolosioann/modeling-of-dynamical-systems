...
όπου $e_1 = theta - hat(theta)$ το σφάλμα θέσης και $(lambda_1, lambda_2) > 0$ τα κέρδη. Ορίζουμε επιπλέον το σφάλμα ταχύτητας $e_2 = dot(theta) - hat(x)_2$ και τα παραμετρικά σφάλματα $tilde(theta)_i = hat(theta)_i - theta_i^*$. Αφαιρώντας την @eq-pend-est από την @eq-pend-ss παίρνουμε:
...
Θεωρούμε τη συνάρτηση Lyapunov:
...
με $gamma_1, gamma_2 > 0$ κέρδη προσαρμογής. Το βάρος $lambda_2 slash 2$ μπροστά από το $e_1^2$ (αντί για $1 slash 2$) είναι χρήσιμο ώστε να "φύγουν" οι όροι ($e_1 e_2$) στην παράγωγο. Παραγωγίζοντας:
...
Συνεπώς $e_1, e_2, tilde(theta)_1, tilde(theta)_2 in L_oo$ και $e_1 in L_2$. Mε εφαρμογή του Λήμματος Barbalat προκύπτει $lim_(t->oo) e_1(t) = 0$. Η ασυμπτωτική σύγκλιση των παραμέτρων $hat(theta)_i -> theta_i^*$ απαιτεί επιπλέον η είσοδος να ικανοποιεί την ΣΕΔ ως προς το διάνυσμα παλινδρόμησης $phi.alt(t) = [sin(theta(t)), dot(theta)(t)]^T$. Για είσοδο $u(t) = 0.5 sin(t)$ ενδεχομένως η ΣΕΔ να μην ικανοποιείται πλήρως, οπότε τα $hat(l), hat(c)$ να παραμένουν φραγμένα αλλά να μην συγκλίνουν ασυμπτωτικά στις πραγματικές τιμές.

== Επίδραση Μετρητικού Θορύβου

Στο υπο-ερώτημα (β) στις μετρήσεις $theta(t), dot(theta)(t), u(t)$ προστίθεται ημιτονοειδής θόρυβος $eta(t) = eta_0 sin(20 pi t)$. Αντικαθιστώντας τα θορυβώδη σήματα $theta_m = theta + eta, dot(theta)_m = dot(theta) + eta, u_m = u + eta$ στην @eq-pend-est, η @eq-pend-err τροποποιείται ώστε να εμφανίζονται επιπλέον όροι εξαρτώμενοι από το $eta(t)$. Η παράγωγος Lyapunov παύει να είναι αρνητικά ημιορισμένη και παίρνει τη μορφή:

$ dot(V) = -lambda_1 lambda_2 e_1^2 + "όροι εξαρτώμενοι από " eta $

Το σύστημα παραμένει ευσταθές, αλλά οι εκτιμήσεις $hat(l), hat(c)$ λογικά θα εμφανίζουν:

- bias λόγω της σύζευξης του θορύβου με τους όρους $sin(tilde(x)_1), tilde(x)_2$ στον νόμο προσαρμογής.
- Ταλαντώσεις γύρω από την biased τιμή, με πλάτος που μεγαλώνει ανάλογα με το $eta_0$ και τα κέρδη $gamma_1, gamma_2$.

Για την ποσοτικοποίηση της επίδρασης, θα παρουσιαστεί γραφική παράσταση του τελικού σφάλματος εκτίμησης $|e_l|, |e_c|$ συναρτήσει του πλάτους $eta_0$, ώστε να φανεί η εξάρτηση της μεροληψίας από την ένταση του θορύβου.

== (α) Αποτελέσματα — Χωρίς Θόρυβο

Η προσομοίωση πραγματοποιήθηκε στο MATLAB με χρήση της `ode45`, για τις τιμές $l = 1.5$ m, $c = 0.5$, κέρδη προσαρμογής $gamma_1 = 5, gamma_2 = 2$, κέρδη $lambda_1 = 20, lambda_2 = 100$, χρονικό ορίζοντα $T = 40$ s, αρχική γωνία $theta(0) = 0.2$ rad και αρχικές εκτιμήσεις $hat(theta)_1 (0) = 0.9 dot g slash l approx 5.886$, $hat(theta)_2 (0) = 0.9 dot c = 0.45$, δηλαδή $10%$ κάτω από τις αληθινές τιμές.

#figure(
  image("../outputs/theta_(a) no noise.svg", width: 95%),
  caption: [Σύγκριση πραγματικής γωνίας $theta(t)$ και εκτιμώμενης $hat(theta)(t)$, χωρίς θόρυβο.],
) <fig-p2a-theta>
...
Tο $theta(t)$ εκτιμάται με υψηλή ακρίβεια (Το σφάλμα $|e_theta|$ είναι της τάξης $10^(-3)$ rad). Τα σφάλματα των παραμέτρων $e_l, e_c$ δεν συγκλίνουν στο μηδέν, παρουσιάζοντας αντίθετα μια αργή ολίσθηση, το μεν $e_l$ μακριά από το 0, ενώ το $e_c$ προς το 0.
...
Ο θόρυβος φαίνεται να επιδεινώνει ελαφρά την ακρίβεια για μικρά πλάτη θορύβου, αλλά δεν αλλάζει ποιοτικά τη συμπεριφορά του συστήματος.
